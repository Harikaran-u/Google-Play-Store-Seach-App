...
class AppStore extends Component {
  ...
  changeApp = appTabId => {
    this.setState({activeTabId: appTabId})
  }
  ...
  render() {
    ...
    const currentApps = this.getFilteredItems()
    console.log(currentApps)

    const searchResults = currentApps.filter(eachApp =>
      eachApp.appName.toLowerCase().includes(searchInput.toLowerCase()),
    )

    const homePage = (
      <div className="apps-home-cont">
        <div className="title-cont">
          <h1 className="apps-head">App Store</h1>
          <div className="search-input-cont">
            <input
              type="search"
              placeholder="Search"
              className="input-el"
              onChange={this.selectApps}
            />
            <img
